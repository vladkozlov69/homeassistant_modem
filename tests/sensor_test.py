...
class GsmModemSmsSensorTest(unittest.IsolatedAsyncioTestCase):
    def test_init(self):
        mock_gateway = Mock(spec=Gateway)
        mock_gateway.get_sms_messages.return_value = []
        sensor = GsmModemSmsSensor(Mock(bus=Mock(), data={'mm_modem': {'MM_MODEM_GATEWAY': mock_gateway}}),
                                   Mock(data={}))
        assert sensor.name == 'GSM Modem SMS'
        assert sensor.state is None

    async def test_update_new_messages(self):
        mock_gateway = Mock(spec=Gateway)
        ...
        sensor = GsmModemSmsSensor(Mock(bus=mock_bus, data={'mm_modem': {'MM_MODEM_GATEWAY': mock_gateway}}),
                                   Mock(data={}))
        ...
        calls = [
            call('logbook_entry',
                 {'name': 'GSM Modem SMS', 'message': 'msg text', 'domain': 'mm_modem',
                  'entity_id': 'mm_modem.incoming_sms'}, context=None),
            call(event_type='mm_modem_incoming_sms',
                 event_data={'path': '/m/p/1', 'number': '01234', 'timestamp': 'dd-mm-2025', 'text': 'msg text'}),
            call('logbook_entry',
                 {'name': 'GSM Modem SMS', 'message': 'msg text2', 'domain': 'mm_modem',
                  'entity_id': 'mm_modem.incoming_sms'}, context=None),
            call(event_type='mm_modem_incoming_sms',
    ...
    async def test_update_new_messages_skip_existing(self):
        mock_gateway = Mock(spec=Gateway)
        mock_bus = Mock(spec=EventBus)
        sensor = GsmModemSmsSensor(Mock(bus=mock_bus, data={'mm_modem': {'MM_MODEM_GATEWAY': mock_gateway}}),
                                   Mock(data={}))
        ...
        calls = [
            ...
            call(event_type='mm_modem_incoming_sms',
                 event_data={'path': '/m/p/2', 'number': '01234', 'timestamp': 'dd-mm-2025', 'text': 'msg text2'}),
            call('logbook_entry',
                 {'name': 'GSM Modem SMS', 'message': 'msg text3', 'domain': 'mm_modem',
                  'entity_id': 'mm_modem.incoming_sms'}, context=None),
            call(event_type='mm_modem_incoming_sms',
